...
def start_request(jobid, message):
    try:
        ...
        print("Message created, thread = ", new_thread)
        ...
        with chats_lock:
            threads[new_thread.id] = {
                'run_id': run_response.id,
                'status': 'processing'
            }

        return jsonify({'thread_id': new_thread.id})
...
def process_request(threadid, jobid, message):
    with chats_lock:
        ...
        # Process the message in the existing thread
        try:
            message_response = client.beta.threads.messages.create(
                thread_id=threadid,
                role="user",
            ...
            print("Message created for existing thread")
            ...
            print("Run created for existing thread")
...
@chat_blueprint.route('/status/<threadid>', methods=['GET'])
def message_status(threadid):
    with chats_lock:
        if threadid not in threads:
            return jsonify({'error': 'Thread not found'}), 404
